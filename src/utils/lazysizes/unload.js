  ...
  function init () {
    ...
    if (config.autoUnload) {
      docElem.addEventListener('load', function (e) {
        if (e.target.naturalWidth * e.target.naturalHeight > config.unloadPixelThreshold && e.target.className && e.target.className.indexOf && e.target.className.indexOf(window.lazySizesConfig.loadingClass) !== -1 && e.target.className.indexOf(window.lazySizesConfig.preloadClass) === -1) {
          lazySizes.aC(e.target, window.lazySizesConfig.unloadClass)
        }
      }, true)
    }
