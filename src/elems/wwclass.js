  ...
  static dep (depArray, errhandler) {
    return wrap(function (method, args, type, key, Target) {
      ...
      return new Promise(function (resolve, reject) {
        loadjs.load(depArray, {
          success: function () {
            if (type === 'class') {
              Target.prototype._deps = Array.prototype.slice(arguments, 0)
            } else {
              args.concat(Array.prototype.slice(arguments, 0))
            }
          ...
          error: function (errFile) {
            ...
            if (typeof (errFunc) === 'function') {
              errFunc.call(self, errFile)
            } else {
              if (cfg.debug) {
                console.error(`元素依赖${key}加载文件“${errFile}”失败`)
              }
              EE.emit('error', 'elem.depfailed', errFile)
            }
          }
        })
      })
