...
- upload: upload($form,context);如果实例处于一个form表单中，在表单每次上传开始前调用，如果需要，负责构建隐藏的input并设置/更新值以上传数据。对于文件类的上传，可以使用input[type=file]，并为$.val(blob-util)通过[](https://github.com/nolanlawson/blob-util)来设置值，启用模拟文件的上传。如果需要用户交互，需要自行设置<label for='id'>来允许用户交互。唯一id可以使用[wwjs.ui.uniq](module-utils_ui.html#~uniq)来创建。这一函数可以返回promise,此时promise结束之后才会继续上传。即便使用了upload函数，依然可以使用data-upload(值为action字符串)来指定上传处理，本处理在upload之后执行。
